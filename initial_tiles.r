...
# returns initial tiles, that is tiles which are active since the first year and have enough data to cover the time span properly (defined by measurement_coverage_threshold)
get_initial_tiles = function(path_weather_files, 
                             sdf_stations, 
  ...
  print( paste0("Determining initial tiles (", year_start, "-", year_end ,"). ..."))
  ...
  for(i in (year_start:year_end)) {
    ...
    sdf_tiles_per_year <- get_active_tiles_of_year(sdf_weather_data, 
                                                   year_span = year_span, 
                                                   measurement_coverage_threshold = measurement_coverage_threshold)
    
    spark_write_csv(sdf_tiles_per_year, 
                    path = path_tmp_tiles_yearly, 
                    mode = {if (append) "append" else "overwrite"})#, 
    append <- TRUE
    print( paste0( "Wrote temporary data (Active tiles) for year ", i, "."))
  }
  sdf_tiles_per_year <- spark_read_csv(sc, "tmp", path_tmp_tiles_yearly)
  ...
  sdf_tiles_initial <- get_active_tiles_of_years(sdf_tiles_initial, 
                                                 year_span = year_span, 
                                                 measurement_coverage_threshold = measurement_coverage_threshold)
...
get_active_tiles_of_year = function( sdf_weather_data, 
                                     year_span, 
                                     measurement_coverage_threshold) {
  sdf_weather_data %>% 
...
get_active_tiles_of_years = function( sdf_weather_data, 
                                      year_span, 
                                      measurement_coverage_threshold) {
  sdf_weather_data %>%
    group_by(Tile_Id) %>% 
